  ...
  return (
    <div className='bg-white p-6 min-h-[calc(100vh-60px)]'>
      <div className='pb-7 pt-1 text-xl flex justify-between items-center'>
        ...
        <Button variant='transparent' color='black' className='text-sm' rightSection={<IconArrowRight size={16} />}>
          View all
        </Button>
      </div>
      {selectedPrefab && (
      ...
      <SimpleGrid cols={4} spacing='lg'>
        {prefabs.map(prefab => {
          ...
          return (
            <MarketplaceCard key={prefab.id} image={prefab.image} user={user} name={prefab.name} price={prefab.price} />
          );
        })}
      </SimpleGrid>
      ...
      {loading ? (
        ...
        <SimpleGrid cols={4} spacing='lg'>
          {components.map(component => {
            ...
            return (
              <MarketplaceCard key={component.id} image={component.thumbnail_url} user={user} name={component.name} price={component.price || 0} />
            );
          })}
        </SimpleGrid>
      )}
