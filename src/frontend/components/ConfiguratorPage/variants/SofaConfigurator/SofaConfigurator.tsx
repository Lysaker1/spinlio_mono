...
const SofaConfigurator: React.FC<{ isAuthenticated: boolean }> = ({ isAuthenticated }) => {
  const {
    session,
    viewport,
    isLoading,
    ...
    setViewport,
    handleDesignSelect,
  } = useConfiguratorState(CONFIGURATOR_TYPES.SOFA);
  ...
  return (
    <ErrorBoundary>
    <Header session={session} viewport={viewport} configuratorType={CONFIGURATOR_TYPES.SOFA} />
    <div className="grid grid-cols-[auto_1fr] grid-rows-[auto_1fr_auto] grid-areas-[sidebar_header_sidebar_viewer_sidebar_panel] h-screen overflow-hidden fixed w-full top-0 left-0">
      <Sidebar
        configuratorType={CONFIGURATOR_TYPES.SOFA}
      ...
      >
        <MyDesigns 
          ...
          currentConfiguratorType={CONFIGURATOR_TYPES.SOFA}
        />
      </Sidebar>
        ...
        <div className="w-full h-screen flex justify-center items-center overflow-hidden">
          <Suspense fallback={null}>
            <ShapeDiverViewer
              ...
              ticket={configuratorConfigs.sofa.ticket}
            />
          </Suspense>
        </div>
          ...
          <div className={`fixed flex flex-col gap-2 h-screen bottom-0 top-0 right-0 pr-2 pb-2 pt-20 w-80 rounded-3xl transition-transform duration-500 ease-in-out`}>
            ...
            <div className='bg-white w-full h-32 rounded-3xl p-6 flex flex-col justify-between'>
              ...
              <BuyButton configuratorType={CONFIGURATOR_TYPES.SOFA} />
            </div>
          </div>
          )}
        </div>
